...
- Context ACL (F3): real `ContextPort` implementation — a shared `ClaudeCodeRunner` base (subprocess invocation of the `claude` CLI with timeout/retry and transient/fatal/logic failure classification into `StageError`) plus `ClaudeContextAdapter`, which runs a two-pass collect-then-review over the workspace (initial JSON-structured relevant-files/summary collection, then a self-review pass that degrades gracefully to the first pass on non-JSON output or reviewer failure) and persists the result as a Markdown file under `~/.autodev/workitems/<id>/`, outside the git worktree. Covered by a `ContextPort` contract test (`tests/adapters/test_context_contract.py`) that runs the same assertions against both `FakeContext` and the real adapter (fake runner injected), plus a `@pytest.mark.live` smoke test (skipped unless `AUTODEV_LIVE=1`) that exercises the real `claude` CLI end to end.
