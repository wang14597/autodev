...
平台控制台以领域聚合根 **Project（项目）** 为中心，**两步流程**：先建项目（登记一个 git 仓库或本地路径 + **跟踪分支**（可留空取仓库默认），此时做一次性 setup：`git fetch` 同步远端 + 探测分支）→ 在项目下创建多个工作项（WorkItem，只填需求）。工作项后台自动驱动 需求录入→分诊→上下文收集（真调 Claude）→ 详情页展示完整生命周期与产出的上下文简报；其 worktree 以项目跟踪分支的最新（`origin/<branch>`）为基点。同一项目下的工作项**共享一次性 setup**（不再重复判断分支/拉远程）。点项目「刷新」会重新 `git fetch` 把远端更新同步到本地。导航：项目列表 → 项目详情（其工作项 + 在此新建 + 刷新 + 删除）→ 工作项详情。当前跑到**评审**阶段为止，开发及之后（开发/验收…）标"待建设"，随 F6–F8 接入。工作项的 `autonomy_enabled` 是**节奏开关**：开＝自动挡（连续跑到平台能力边界）；关＝手动挡（只读收集段仍自动跑完，之后每个阶段由你点「推进」走一步）。真调 Claude 需在能访问内网网关的环境（VPN）里运行。
...
- **8 个端口已真实实现**：
  ...
  - ReviewPort → ClaudeReviewAdapter（Claude Code 只读单遍核对代码，产出最终方案文档，F5）
- **3 个端口当前为假实现**（mock/stub）：Execution/Verification/Delivery，真实 ACL 见下方"计划中"
- 另有驱动侧适配器：项目控制台（前端 `frontend/` + 后端 `src/autodev/webapp/`），以 Project 为中心驱动工作项走 需求录入→分诊→上下文→方案→评审；节奏由 `autonomy_enabled` 决定（自动挡连续跑到能力边界，手动挡在收集段之后每阶段等人点「推进」），风险门禁照常挂起为 `WAIT_HUMAN` 交人审

✅ **测试覆盖**
- 后端 **372 个测试通过、4 个跳过**（跳过用例需 `AUTODEV_LIVE=1` + 可用 claude/内网网关触发）；前端 vitest **86 个测试通过**
...
✓ DesignPort              → ClaudeDesignAdapter（Claude Code 只读单遍产出方案，F4）
✓ ReviewPort              → ClaudeReviewAdapter（Claude Code 只读单遍核对代码产出最终方案，F5）
