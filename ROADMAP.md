...
**时间线**：2026-07-15 - 2026-07-19  
**状态**：进行中  
**依赖**：切片 1 完成（✓ 已满足）

| 任务 | 描述 | 状态 | 负责 |
|------|------|------|------|
| B1 | README + ROADMAP（项目门面） | ▢ 进行中 | — |
| B2 | 架构图（5 张 Mermaid，含系统/限界/状态机/时序/数据模型） | ▢ 待做 | — |
| B3 | ADR 决策记录（轻量引擎/版本化/中性化） | ▢ 待做 | — |
| B4 | 协作与治理（CONTRIBUTING/SECURITY/LICENSE/CHANGELOG/MR 模板/CODEOWNERS） | ▢ 待做 | — |
| B5 | 质量门禁（ruff/mypy/pre-commit/.gitlab-ci.yml） | ▢ 待做 | — |
...
**进行中**：项目基线的 CI/CD 质量门禁（见 B5 任务）

**后续**：
- 本地 pre-commit hook：快速 lint / type check
- GitLab CI：自动化测试、部署
