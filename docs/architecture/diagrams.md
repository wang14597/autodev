...
`WorkItem` 是唯一聚合根，`artifact_versions` 按阶段键存版本列表（只追加不改）；`history` 记录每次合法转移；值对象 `Requirement`/`RepoRef`/`AutonomyDial`/`RetryLedger`/`Cost` 为聚合的不可变属性。8 个产物类型（`artifacts.py`）彼此之间没有共同的运行时基类，图中按各自的 `artifact_versions` 键名与聚合关联，仅表达逻辑上的“版本化产物族”，不代表代码里的继承关系。
