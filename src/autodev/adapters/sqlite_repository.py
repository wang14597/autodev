...
from autodev.domain.work_item import StateTransition, WorkItem
...
class SqliteWorkItemRepository:
    ...
    def save(self, work_item: WorkItem) -> None:
        ...
        with self._conn() as c:
            c.execute(
                "INSERT INTO work_items(id, state, data) VALUES(?,?,?) "
    ...
    def get(self, work_item_id: WorkItemId) -> WorkItem:
        with self._conn() as c:
            row = c.execute(
                "SELECT data FROM work_items WHERE id=?", (work_item_id.value,)
            ).fetchone()
        if row is None:
...
def _to_dict(wi: WorkItem) -> dict:
    return {
        ...
        "requirement": {
            ...
            "target_repo": wi.requirement.target_repo,
        ...
        "history": [
            [h.from_state.name, h.to_state.name, h.reason, h.at.isoformat()] for h in wi.history
        ],
...
def _from_dict(d: dict) -> WorkItem:
    wi = WorkItem(
        ...
        history=[
            StateTransition(WorkflowState[a], WorkflowState[b], reason, datetime.fromisoformat(at))
            for (a, b, reason, at) in d["history"]
        ],
...
def _artifact_to_dict(a: object) -> dict:
    ...
    if isinstance(a, VerificationArtifact):
        return {
            ...
            "reasons": list(a.verdict.reasons),
